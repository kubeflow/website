...
if you are using Basic Authentication when deploying AWS Kubeflow, we have below suggestions for you:

* Change Default password in configuration file - The configuration file ({{% config-uri-aws-standard %}}) contains:
    ```
    spec:
      auth:
       basicAuth:
    ...
    ```
    By default, we have username as `admin@kubeflow.org` and password as `12341234`. You should change this configuration to secure your kubeflow deployment.
...
If you are using {{% kf-latest-version %}} config file, run following command to get Kubeflow service endpoint host name and copy link in browser.
...
To secure enterprise level installation, use {{% config-uri-aws-cognito %}} and follow the [instructions](/docs/aws/authentication) to add authentication and authorization.
...
# If you want to add a statice user (test@kubeflow.org: 123456789)
