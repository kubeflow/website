...
In the following example we are using SubprocessRunner to interact with Google Cloud services from local machine.
Before executiong this scipt you have to authenticate with [Application Default Credentials](https://cloud.google.com/docs/authentication/provide-credentials-adc). You can use following command: `gcloud auth application-default login`
