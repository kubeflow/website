...
Note that, in order to use Autolog, you must [enable injection](/docs/components/pipelines/operator-guides/mlflow-plugin/#plugin-optional-settings) of [MLflow environment variables](/docs/components/pipelines/user-guides/integrations/mlflow/overview/#mlflow-environment-variables-in-pipeline-end-user-code) for user containers.

### MLflow Environment Variables in Pipeline End-User Code
MLflow environment variable injection has end-user code applications beyond `autolog()`. The following environment variables are available for injection [(if enabled)](/docs/components/pipelines/operator-guides/mlflow-plugin/#plugin-optional-settings):
...
When running in multiuser mode, namespace-level configs are prioritized. Learn more about namespace-level MLflow configuration [here](/docs/components/pipelines/operator-guides/mlflow-plugin/#mlflow-plugin-configuration-in-multi-user-mode). 
...
Note: These overrides are only available through direct API calls, not through the KFP UI or Python SDK. See [Run a Pipeline](/docs/components/pipelines/user-guides/integrations/mlflow/run-a-pipeline/) for examples. 
