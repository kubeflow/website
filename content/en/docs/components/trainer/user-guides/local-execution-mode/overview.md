...
All backends support the same job management operations through the `TrainerClient` interface.
...
- Wider ecosystem support and third-party tools
- Simpler setup with Docker Desktop
- Team familiarity (most developers know Docker)

### When to Use Podman

- Security is a priority (rootless containers, no root daemon)
...
### Step 1: Start with Local Process Backend

Begin with quick local testing:
...
### Step 2: Move to Container Backend

Switch to Docker/Podman for multi-node distributed training:
...
Move to production with the Kubernetes backend:
...
- **[Local Process Backend](./local_process.md)** - For quick local testing without containers
- **[Docker Backend](./docker.md)** - For containerized distributed training with Docker
- **[Podman Backend](./podman.md)** - For secure, rootless containerized training with Podman
