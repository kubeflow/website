...
* **Beta**: [AI Platform Pipelines](#marketplace) provides easy-to-use management
UI for installing and using Kubeflow Pipelines on GCP.
...
1. Do you want to use Kubeflow Pipelines with [multi user support](https://github.com/kubeflow/pipelines/issues/1223)?
    ...
    If yes, wait for the to be released [full Kubeflow](#full-kubeflow) 1.1.
...
: 
  ...
  * Kubeflow Pipelines endpoint is **only auto-configured** for GCP.
  If you deploy on other platforms, you can either access it through
...
:
  ...
  * KFP multi user support is **not available** in standalone, because
  it depends on other Kubeflow components.
...
:
  ...
  * KFP multi user support is **only available** in full Kubeflow. It supports
...
Upgrade Support (**Alpha**)
: In place upgrade is not supported. Reinstall new version while retaining data
is supported without official documentation. You can
[delete a AI Platform Pipelines application](https://cloud.google.com/ai-platform/pipelines/docs/getting-started#clean_up)
**WITHOUT** selecting **Delete cluster** checkbox and reinstall the newer version
in the same cluster and the same namespace. Persisted artifact and database data
are in GCP persistent disks and will be automatically picked up during
reinstallation.
...
: 
  * After deployment, your Kubernetes cluster contains Kubeflow Pipelines only. 
  ...
  Notebooks](https://cloud.google.com/ai-platform/notebooks/docs/).
  * KFP multi user support is **not available** in AI Platform Pipelines, because
  it depends on other Kubeflow components.
