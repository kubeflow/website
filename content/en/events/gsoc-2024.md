...
**If you need to reach out our mentors, join [Kubeflow Slack](https://kubeflow.slack.com/archives/C7REE0ETX) and look up your mentor or find project specific channel on Kubeflow slack**
...
Recently, we implemented a new Python SDK API in Kubeflow Training Operator to easily fine-tune LLM using custom model and datasets providers. Ref proposal for Training Operator: [https://github.com/kubeflow/training-operator/blob/master/docs/proposals/train_api_proposal.md](https://github.com/kubeflow/training-operator/blob/master/docs/proposals/train_api_proposal.md) We want to give user functionality to tune HyperParameters of LLMs using simple Python SDK APIs.
...
### Project 6: ush-based metrics collection for Katib

Open issue: [https://github.com/kubeflow/katib/issues/577](https://github.com/kubeflow/katib/issues/577).

Katib uses a sidecar container to collect training metrics from the Trials once training is complete. Sometimes users don’t want to use sidecars to get the training metrics and directly push metrics to Katib DB using APIs. We want to implement a new API in the Katib Python SDK to allow users to push metrics from their training scripts to the Katib DB.
...
We need to enhance Katib APIs to support various parameter distributions like uniform, log-uniform, qlog-uniform to make Katib more native to other HyperParameter tuning frameworks like Hyperopt.
...
**Mentors**: Matteo Mortari (mmortari@redhat.com), Andrea Lamparelli (alampare@redhat.com)
