...
### Project: MCP Server for Kubeflow SDK

**Components:** Kubeflow SDK, Trainer

**Mentors:** [TBD] @jaiakash

**Contributor:** [TBD]

**Details:**
The Kubeflow SDK allows users with limited Kubernetes knowledge to use standard Python APIs to interact with the Kubeflow ecosystem.
* **Documentation:** [https://sdk.kubeflow.org/en/latest/index.html](https://sdk.kubeflow.org/en/latest/index.html)

We currently use LLMs to create code for jobs, models, etc., but there is no mechanism for the LLM to see the `TrainJob` status or debug a crash loop. We want to extend and improve the Developer Experience (DX) by leveraging modern LLM capabilities with the introduction of a **Model Context Protocol (MCP)** server.

This project aims to initiate the Kubeflow MCP project with first-principle features, enabling users to interact, debug, and get results of their jobs via natural language.

**Features Expected:**
* Expose training logs and status as standard MCP resources.
* Trainer runtime and job management (CRUD operations on TrainJobs).
* LLM-enhanced tuning and validation of TrainJobs before execution.
* Analysis of telemetry and logs (Reference: [Issue #164](https://github.com/kubeflow/sdk/issues/164)).

**Difficulty:** Medium

**Size:** 175 hours (Medium) or 350 hours (Large)
