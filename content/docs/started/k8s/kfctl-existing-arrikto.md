...
If you don't have support for LoadBalancer on your cluster, please
follow the instructions below to deploy MetalLB in [Layer 2
mode](https://metallb.universe.tf/configuration/#layer-2-configuration).
...
b) Ensure that MetalLB has actually allocated an IP for the service:
```bash
...
c) Check the corresponding MetalLB logs:
...
d) Create a pod that will be exposed with the service:
...
e) Ensure that MetalLB has assigned a node to announce the allocated IP:
...
f) Check the corresponding MetalLB logs:
...
g) Check that MetalLB responds to ARP requests for the allocated IP:
...
h) Check the corresponding MetalLB logs:
...
i) Verify that everything works as expected:
...
Get the IP and open it in a browser: `https://<LoadBalancerIP>/`.
...
If the Kubeflow Dashboard is not available at `https://<LoadBalancerIP>` ensure that:

a) the LoadBalancer service for Istio has obtained an external IP, for example:
...
b) the virtual services have been created:
...
c) OIDC auth service redirects you to Dex:

```bash
curl -k https://<LoadBalancerIP>/ -v
...
< HTTP/2 302
