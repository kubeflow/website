...
1. When Kubeflow is running, access the Kubeflow UI at a URL of the form
  ...
  <img src="/docs/images/central-ui.png" 
   ...
   If IAP is skipped, run ```kubectl port-forward -n kubeflow `kubectl get pods -n kubeflow --selector=service=ambassador -o jsonpath='{.items[0].metadata.name}'` 8080:80``` and go to http://localhost:8080/
